...
var logic: Record<string, any> = {};
var write =
  ((typeof console === "object") && (typeof console.log !== "undefined"))
    ? console.log
    : function () {};
...
function Domain(this: any, min, max) {
  this.min = min;
  this.max = max;
}
...
function Constraint(this: any, fn, args, name) {
  this.fn = fn;
  this.args = args || [];
  this.name = name || "";
}
  ...
  return function (p) {
    ...
    if (dz) {
      ...
      if (dx) {
        ...
        if (dy) {
          return p.extend_domain(x, dx).extend_domain(y, dy).extend_domain(
            z,
            dz,
          );
        }
...
function Package(this: any, f, cs, d) {
  this.frame = f;
  this.store = cs;
  this.domains = d;
  this.type = "package";
...
Package.prototype.extend_binding = function (variable, val) {
  return logic.make_package(
    this.frame.extend(
      logic.make_binding(variable, val),
      this.store,
      this.domains,
    ),
    this.store,
...
Package.prototype.extend_domain = function (v, d) {
  ...
  return new Package(
    this.frame,
    this.store,
    this.domains.extend(logic.make_binding(v, d)),
  );
};
...
Package.prototype.toString = function () {
  var str = "{" + (this.frame.is_empty()
    ? ""
    : this.frame.toString() + (this.store.is_empty() ? "" : ", ")) +
    (this.store.is_empty() ? "" : this.store + ", ") + (this.domains.is_empty()
      ? ""
      : this.domains) +
    "}";
  return str;
...
Stream.prototype = {
  ...
  map: function (f) {
    ...
    if (s.is_empty()) {
      ...
      var _rest = s.rest().map(f);
      return logic.make_stream(f(s.first), function () {
        return _rest;
      });
    }
  },
  ...
  extend: function (val) {
    var frame = this;
    return logic.make_stream(val, function () {
      return frame;
    });
  },
  toString: function () {
...
function Binding(this: any, variable, val) {
...
Binding.prototype.toString = function () {
  return (typeof this.variable.name !== "undefined"
    ? this.variable.name
    : "_") + "=" + this.val.toString();
};
...
function has_type(obj, type) {
  return (typeof obj === "object") && (typeof obj.type !== "undefined") &&
    obj.type === type;
}
...
logic.list = function () {
  var l:any = Array.prototype.slice.call(arguments);
  l.type = "logic_list";
  return l;
};
...
logic.win = function (pack) {
  return logic.make_stream(pack || logic.nil);
};
logic.fail = function (pack) {
  return EMPTY_STREAM;
};
...
logic.implies = function (g1: any, g2: any, g3: any) { //g1 -> g2 ; g3
...
var _between = function (a: any, b: any, x: any) {
  if (a > b) {
    return logic.fail;
  } else {
...
logic.between = function (a: any, b: any, x: any) {
...
logic.run = function (g: any, v: any, n: any) {
  ...
  for (var i = 0; i < n && !s.is_empty(); ++i) {
    ...
    if (logic.is_lvar(v)) { //get variable into result
      var v2 = frame.walk(v);
      var _temp:any = logic.is_lvar(v2) ? pack.get_value(v) : v2;
      result.push(_temp);
